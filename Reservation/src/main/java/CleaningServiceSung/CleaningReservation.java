...
import CleaningServiceSung.CleaningRequestCanceled;
...
public class CleaningReservation {
    ...
    public void onPostPersist(){
        CleaningServiceSung.external.Payment payment = new CleaningServiceSung.external.Payment();
        payment.setRequestId(getId());
        payment.setPrice(getPrice());
        payment.setStatus("PaymentApproved");

        try {
        	ReservationApplication.applicationContext.getBean(CleaningServiceSung.external.PaymentService.class)
            	.payRequest(payment);
        } catch(Exception e) {
